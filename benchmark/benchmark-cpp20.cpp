...
struct ThreadContext
{
    ...
    ThreadContext(std::unique_ptr<grpc::CompletionQueue> cq) : grpc_context(std::move(cq)) {}
};
...
void repeatedly_request_unary(grpc::testing::BenchmarkService::AsyncService& service, ThreadContext& thread_context)
{
    co_spawn(thread_context.grpc_context,
             [&]() -> boost::asio::awaitable<void>
             {
                 auto context =
                     std::allocate_shared<UnaryRPCContext>(std::pmr::polymorphic_allocator<>(&thread_context.resource));
                 bool request_ok =
                                       ...
                                       ThreadContext& thread_context)
{
    co_spawn(thread_context.grpc_context,
             [&]() -> boost::asio::awaitable<void>
             {
                 auto context = std::allocate_shared<StreamingCallRPCContext>(
                     std::pmr::polymorphic_allocator<>(&thread_context.resource));
                 bool request_ok =
...
int main()
{
    ...
    std::forward_list<ThreadContext> thread_contexts;
    for (size_t i = 0; i < cpu_count; ++i)
    {
        thread_contexts.emplace_front(builder.AddCompletionQueue());
    }
    ...
    for (size_t i = 0; i < cpu_count; ++i)
    {
        threads.emplace_back(
            [&, i]
            {
                auto& thread_context = *std::next(thread_contexts.begin(), i);
                repeatedly_request_unary(service, thread_context);
                repeatedly_request_streaming_call(service, thread_context);
                thread_context.grpc_context.run();
            });
    }
